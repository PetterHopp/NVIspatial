...
Thank you for considering contributing to `NVIspatial`.

`NVIspatial` is one of several packages assembled under the name
...
Using `NVIspatial` and need help? Browse the package help to see if you
...
Have an idea for a new `NVIspatial` feature? Take a look at the
`NVIspatial` help and [issue
list](https://github.com/NorwegianVeterinaryInstitute/NVIspatial/issues)
to see if it isn’t included or suggested yet. If not, suggest your idea
as an [issue on
GitHub](https://github.com/NorwegianVeterinaryInstitute/NVIspatial/issues/new).
...
Using `NVIspatial` and discovered a bug? Don’t let others have the same
...
1.  Go to `R/` directory in the [code
    repository](https://github.com/NorwegianVeterinaryInstitute/NVIspatial/tree/main/R).
...
-   Please limit the number of package dependencies for `NVIspatial`.
